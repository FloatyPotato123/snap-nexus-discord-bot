...
import { generateDeckcodeString } from "snapdeck";
  ...
  const aiResponse = await fetch("https://router.huggingface.co/v1/chat/completions", {
    ...
    body: JSON.stringify({
      ...
      messages: [{
        role: "user",
        content: [
          { type: "text", text: "List the 12 Marvel Snap card names in this deck. Respond ONLY with a raw JSON array of strings." },
...
function matchCardsToDatabase(extractedNames) {
  ...
  for (const raw of extractedNames) {
    ...
    const exact = cardDatabase.find(c => c.name.toLowerCase() === clean.toLowerCase());
    if (exact) {
      matched.add(exact);
      continue;
    }

    const bestMatch = stringSimilarity.findBestMatch(clean, ALL_CARD_NAMES).bestMatch;
    if (bestMatch.rating > 0.5) {
      matched.add(cardDatabase.find(c => c.name === bestMatch.target));
    }
  }
...
async function processDeckImage(channel, imageUrl, messageId) {
  ...
  try {
    ...
    const code = generateDeckcodeString(sorted);

    await status.delete().catch(() => {});
    await channel.send({ content: `\`${code}\``, reply: { messageReference: messageId } });
  } catch (error) {
